...
test('AppCard renders correctly with parsed values', () => {
    const tree = renderer.create(<AppCard name = "Bob's Barbeque" city = "Sydney" date = "10/02/2021" category="BBQ" user="User01"/>).toJSON();
    expect(tree).toMatchSnapshot();
});

test('AppIcon renders an icon correctly', () => {
    const tree1 = renderer.create(<AppIcon name="logout" size={50} iconCol="red" backgroundColor="blue"/>).toJSON();
